...
**Sophon − truncate (equal budget): +1.2 pts** (95% CI [-6.0, +7.9], n=51)  
**Sophon − oracle (loss vs full context): -46.3 pts** (95% CI [-55.5, -36.9], n=51)

## Per-op breakdown

| op | n | recall oracle | recall sophon | recall truncate | sophon−trunc | sophon savings |
|---|---|---|---|---|---|---|
| history | 20 | 69.4% | 4.6% | 0.0% | +4.6 | 76.7% |
...
- **The 67% token saving is real but not free**: compressing to 1/3 budget costs **46 pts of key-fact recall** vs the full context (69% → 22%). This is the cost the token-only headline hid.
- **At equal budget, Sophon ≈ naive truncation** (+1.2 pts, CI spans 0). The compression is not meaningfully "smarter" than a dumb cap on average — it wins modestly on `prompt` (section selection), loses on `output`, and ties on `history`.
