...
static SEARCH_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(grep|rg|ripgrep|ag|ack)\b").unwrap());
...
mod tests {
    ...
    fn detects_test_runners() {
        assert_eq!(
            detect_command_family("cargo test -p foo"),
            CommandFamily::TestRunner
        );
        assert_eq!(
            detect_command_family("pytest tests/"),
            CommandFamily::TestRunner
        );
        assert_eq!(
            detect_command_family("go test ./..."),
            CommandFamily::TestRunner
        );
        assert_eq!(
            detect_command_family("vitest run"),
            CommandFamily::TestRunner
        );
    }
    ...
    fn detects_build() {
        assert_eq!(
            detect_command_family("cargo build --release"),
            CommandFamily::Build
        );
    ...
    fn detects_filesystem() {
        ...
        assert_eq!(
            detect_command_family("tree -L 2"),
            CommandFamily::Filesystem
        );
    }
    ...
    fn detects_search() {
        assert_eq!(
            detect_command_family("grep -rn TODO"),
            CommandFamily::Search
        );
    ...
    fn detects_docker() {
        ...
        assert_eq!(
            detect_command_family("kubectl get pods"),
            CommandFamily::Docker
        );
    }
    ...
    fn detects_cloud() {
        ...
        assert_eq!(
            detect_command_family("gcloud projects list"),
            CommandFamily::Cloud
        );
    }
    ...
    fn fallback_to_generic() {
        ...
        assert_eq!(
            detect_command_family("curl example.com"),
            CommandFamily::Generic
        );
    }
}
